...
# endif
	...
	alignas(4) __attribute__((section(".DmaState"))) static volatile uint8_t sendData[12];

	// Buffer for the message we receive when reading data. The first 4 or 12 bytes bytes are our own transmitted data.
	alignas(4) __attribute__((section(".DmaState"))) static volatile uint8_t receiveData[20];
